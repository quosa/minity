...
bool renderOLD(minity::old_scene scene, minity::rasterizer &rasterizer)
{
    renderStats stats{};
    ...
    for (auto face : model.faces)
    {
        ...
        for (int idx : face)
        {
            processVerticesOLD(model, idx, vects, norms, texc,
...
void softwareEngine::run(minity::old_scene scene)
{
    ...
    while (isRunning(&inputTranslation, &inputRotation, &inputChange))
    {
        ...
        if (!(g_config->renderOnChange && !inputChange))
        {
            ...
            bool ok = renderOLD(scene, rasterizer);
            if(!ok)
            {
                std::cerr << "rendering failed - exiting" << std::endl;
                break;
